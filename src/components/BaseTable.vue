  ...
  <div class="bg-white rounded-lg shadow-xl relative">
    ...
    <table class="w-full text-sm text-left text-gray-700">
      <thead class="bg-gray-100 text-[#979899] uppercase text-xs">
        <tr>
          <th v-for="(header, index) in headers" :key="index" class="px-6 py-5 font-medium">
            {{ header.label }}
          </th>
      ...
      <tbody>
        ...
        <tr v-else v-for="(item, index) in items" :key="index" class="border-b border-[#E8E8E8] hover:bg-gray-50">
    ...
    <div class="flex items-center justify-end gap-2 px-6 py-4 text-sm rounded-b-lg">
      <button 
        :class="['px-3 py-2 border border-[#E9E9E9] rounded-md  flex items-center ' ,
      ...
      <!-- <button v-for="page in pagination.total" :key="page" @click="changePage(page)" :class="[
      ...
      <div v-for="(p, index) in getPages(pagination)" :key="index">
        ...
        <button 
          ...
          :class="['px-3 py-2 border border-[#E9E9E9] rounded-md',
